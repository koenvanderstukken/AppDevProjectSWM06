    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                ...
                <li class="current"><a href="GegevensBijwerkenStagecoordinator.php">Gegevens bijwerken</a></li>
            </ul>
        </div>
    </div>
</nav>
<div class="container">
    ...
    <form id="gegevensForm">
        <div id="gegevens">
            ...
            <div class="form-group">
                <label for="nummer">Lector nummer: *</label>
            ...
            <div class="form-group">
                ...
                <input type="email" name="Schoolemail" id="schoolemail" class="form-control" required
                       pattern=".*[a-zA-Z0-9_\.\+-]+@pxl+\.[a-zA-Z0-9-\.]+.*">
            </div>
            <div class="form-group">
...
<script>
    var stagecoordinator;
    $(document).ready(function () {
        $.ajax({
            url: BaseURL+"/stagecoördinator/" + sessionStorage.getItem('userId'),
            type: "Get",
            dataType: "json",
            async: false,
        ...
        $("#gegevensForm").submit(function () {
            stagecoordinator.Achternaam = $('#achternaam').val();
            stagecoordinator.Voornaam = $('#voornaam').val();
            stagecoordinator.Nummer = $('#nummer').val();
            stagecoordinator.Telefoonnummer = $('#telefoonnummer').val();
            stagecoordinator.SchoolMail = $('#schoolemail').val();
            stagecoordinator.Email = $('#email').val();
            $.ajax({
                url: BaseURL+"/stagecoördinator/" + sessionStorage.getItem('userId'),
                type: "PUT",
                data: stagecoordinator,
                async: false,
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('Authorization', 'Bearer ' + sessionStorage.getItem('tokenKey'));
                },
